...
#### Add best correlation
cor_max_df_sub <- cor_max_df %>%
  ungroup() %>%
  dplyr::select(Country, keyword_en,
                time_lag_cases_cor_max, time_lag_death_cor_max,
                cor_cases_new_max, cor_death_new_max)

gtrends_df <- merge(gtrends_df, cor_max_df_sub, by = c("Country", "keyword_en"))
