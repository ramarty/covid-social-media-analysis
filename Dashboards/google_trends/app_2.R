...
server = (function(input, output, session) {
  ...
  output$line_graph <- renderPlot({
    ...
    if(F){
      ...
      #gtrends_df <- gtrends_df[gtrends_df$geo %in% c("US", "BR", "FR", "ES"),]
      #cor_df     <- cor_df[cor_df$geo %in% c("US", "BR", "FR", "ES"),]
      ...
      input <- data.frame(select_covid_type = "Cases",
                          select_keyword = "Loss of Smell")
      
      p_line <- ggplot(gtrends_sub_df, aes(x = date)) +
      ...
      p_cor <- cor_df %>%
        ggplot() +
        geom_col(aes(x = time_lag, y = cor_cases_new, fill = cor_cases_new)) + 
      ...
      saveRDS(p_all, "~/Desktop/graph.Rds")
    }
    
    
    p_all <- readRDS("~/Desktop/graph.Rds")
