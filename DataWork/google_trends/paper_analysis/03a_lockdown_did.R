...
# Days since lockdown, all years -----------------------------------------------
gtrends_df_i <- gtrends_df %>%
  dplyr::filter(keyword_en %in% "boredom") 

lm(hits_ma7 ~ year2020 + 
     days_since_lockdown_min_yearcurrent_post + 
     days_since_lockdown_min_yearcurrent_post_X_year2020, data = gtrends_df_i) %>%
  summary()

# y ~ x1 + x2 | f1 + f2 | (Q|W ~ x3+x4) | clu1 + clu2 
felm(hits_ma7 ~ year2020 + 
       days_since_lockdown_min_yearcurrent_post + 
       days_since_lockdown_min_yearcurrent_post_X_year2020 | mm_dd | 0 | 0, data = gtrends_df_i)

gtrends_df_i$mm_dd


head(gtrends_cases_df)
