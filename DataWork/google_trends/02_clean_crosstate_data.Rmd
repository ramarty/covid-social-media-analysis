---
title: "02_Clean_Merge_Cross_States"
output: html_document
---

#Intro and importing data 

This file cleans the data of covid-19 cases and google searches across states in Brazil.
It then creates new variables that will be useful in the analysis. 

```{r}
#load libraries
library(tidyverse)
library(gridExtra)
library(lubridate)

#user's file path
if(Sys.info()[["user"]] == "wb537287") dropbox_file_path <- "/Users/wb537287/Dropbox/COVID Social Media Analysis"
if(Sys.info()[["user"]] == "manuelramos") dropbox_file_path <- "~/Dropbox/COVID Social Media Analysis"

##import datasets

#google searches data
states_df <-
  read.csv(file.path(dropbox_file_path,"Data/google_trends/RawData/brazil_extract_crosstate.csv"), encoding = "UTF-8")

#population data of Brazil
state_pop_data <-  
  read.csv(file.path(dropbox_file_path, "Data/city_population/FinalData/brazil_state_pop.csv"))

#admin cases of covid-19
admin_data <- 
  read.csv(file.path(dropbox_file_path, "Data/brazil_admin_data/brazil_covid19_200419.csv"), encoding = "UTF-8")
```

#Cleaning 

We clean the state name and create a new variable for it, "location"
```{r}
states_df <- 
  states_df %>% 
  filter(unit_level == "region") %>% 
  mutate(location = str_replace(location, "State of ", "")) %>% 
  rename(state = location)
```


We create the english version of state names by removing accents. This is the variable "state_en"
```{r}
states_df <-   
  states_df %>% 
  mutate(state_en = stringi::stri_trans_general(state, "Latin-ASCII") %>% str_to_upper())
```


We put it in the same form as the population data (english and capitalized) to facilitate the merge. 
We also convert the date var into a date format
```{r}
admin_data <- 
  admin_data %>% 
  mutate(
    state_en = stringi::stri_trans_general(state, "Latin-ASCII") %>% str_to_upper(), 
    date = as.Date(date)
  )
```

#Merge

We merge the admin data with the population data 
```{r}
admin_data <- 
  admin_data %>% 
  left_join(
    state_pop_data, 
    by = c("state" = "State")
  )
```


We also merge it with the google searches data
```{r}
states_admin_df <- 
  states_df %>% 
  left_join(
    admin_data, 
    by = c("state" = "state")
  )
```

#We create categories for key words

We create 5 main categories: consequences, resources, symptoms, prevention, virus, and 1st person searches (self)
```{r}
states_admin_df <- 
  states_admin_df %>% 
  mutate(
    categories = case_when(
      keyword %in% 
        c("abuso", "abuso sexual", "desemprego",  "dívida", "Educação online",
          "psicologia", "terapia" ) ~ "consequences",
      keyword %in% 
        c("cama de hospital", "desinfetantes", "hidroxicloroquina", "cloroquina",
          "máscara facial", "termômetros", "ventiladores", "medicos", 
          "profissionais de saúde") ~ "resources",
      keyword %in% 
        c( "dificuldade ao respirar", "dor nos olhos", "falta de cheiro", 
           "febre", "febre alta valor","perda de olfato", "tosse",
           "sintomas do coronavirus", "teste de coronavírus") ~ "symptoms",
      keyword %in% 
        c("distância social","fique em casa", "lavar as mãos", "Isolamento social", 
          "isolamento vertical") ~ "prevention", 
      keyword %in% c("coronavirus", "covid", "ajuda do coronavírus") ~ "virus", 
      keyword %in% 
        c("como tratar o coronavírus", "Estou com falta de ar",
        "estou com febre", "Eu fico em casa", "eu tenho coronavírus",
        "Perdi o olfato", "quais são os sintomas do coronavírus", "volta brasil"
        ) ~ "in_1st_person"
      )
  )
```

#Create new variables: case rate, death rate, and case fatality

We create variables for case rate, death rate, and case fatality
```{r}
states_admin_df <- 
  states_admin_df %>% 
  mutate(
    case_rate = cases*100000/estimate_2018_state, 
    death_rate = deaths*100000/estimate_2018_state, 
    fatalities_per_case = deaths/cases
  )
```

We remove na's with 0 for the case fatality
```{r}
states_admin_df <- 
  states_admin_df %>% 
  mutate(fatalities_per_case = if_else(is.na(fatalities_per_case), 0, fatalities_per_case))
```

#Export the cleaned data 

```{r}
saveRDS(states_admin_df, file.path(dropbox_file_path, "Data/google_trends/FinalData/brazil_crosstates_clean.Rds"))
write.csv(states_admin_df, file.path(dropbox_file_path, "Data/google_trends/FinalData/brazil_crosstates_clean.csv"), row.names = F)
```

